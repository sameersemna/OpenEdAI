...
bash "$generator" "$report_path" "$contract_json" "$summary_json" "$trend_json" "$verdict_json" "$consistency_json" "$kpi_json" "$checksums_path"
bash "$verifier" "$checksums_path"

printf '\n' >>"$kpi_json"

set +e
bash "$verifier" "$checksums_path" >/dev/null 2>&1
rc=$?
set -e
