...
fixture_path="${tmp_dir}/fast-contract-governance-heartbeat-unexpected-command.yml"
...
needle = '      - name: Validate workflow conventions expected-count behavior\n        run: make verify-workflow-conventions-fast-contract-expected-count-selftest\n'
injected = needle + '\n      - name: Validate workflow conventions unexpected command fixture\n        run: make fast-contract-rogue-selftest\n'
...
if [[ "$rc" == "0" ]]; then
  echo "[workflow-conventions][fail] verifier should fail for unexpected heartbeat command fixture" >&2
  exit 1
fi
...
if ! grep -Fq "$expected" "$output_path"; then
  echo "[workflow-conventions][fail] expected unexpected-command error message not found" >&2
  cat "$output_path" >&2
  exit 1
fi

echo "[workflow-conventions][ok] heartbeat unexpected-command selftest passed"
